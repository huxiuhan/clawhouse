...
export const createPromptApis = (
  ...
  const parseVtokens = async (
    ...
    const resPrompts = await Promise.all(
      insPrompts.map(async (p) => {
        ...
        if (p.type !== "character" && p.type !== "elementum") return null;

        if (p.type === "character") {
          const characterInAssigns = options?.characters?.find(
            (c) => c.name === p.name,
          );
        ...
        if (p.type === "elementum") {
          const elementumInAssigns = options?.elementums?.find(
            (s) => s.name === p.name,
          );
  ...
  const img2imgModes = async (): Promise<RefImageVtoken[]> => {
    return client
      .get<{
        tags: RefImageVtoken[];
      }>("/v1/prompt/full-prompt-tags", {
